                                    ...
                                    if (l_tree.tree) {
                                        ...
                                        if (CommnUtil.in_array(lt, clm)) {
                                            ...
                                            divtree.setAttribute("style",
                                                    "float:left;padding-top:5px;margin-left:5px;text-align:"
                                                            + column[o].align + ";");
...
//             lia.innerHTML = '总共' + totalRecords + '条记录，每页显示' + conf.pageSize
//                     + '条，当前显示第' + pageNow + '页';
//
//
// //          增加分页数下拉框，通过pageSizeArr参数配置
//
//             if(conf.pageSizeArr&&conf.pageSizeArr.length>0){
//                 var btd_1 = document.createElement("td");
...
//                 btddiv.appendChild(divselect);
//
//                 $.each(conf.pageSizeArr,function(i,n){
//                     var optiondom=document.createElement("option");
//                     if(n==params.pageSize){
//                         optiondom.selected="selected";
//                     }
//                     divselect.appendChild(optiondom);
//                     optiondom.value=n;
//                     optiondom.innerHTML=n+'页';
//                 });
...
//                 lia_2.innerHTML = '<i class="fa fa-arrow-left"></i>';
...
//                 lia_7.innerHTML = '<i class="fa fa-arrow-right"></i>';
...
//         };
        ...
        var fenyeDiv = function(divid, jsonData) {
            ...
            var btd_1 = document.createElement("td");
            btd_1.style.textAlign = "right";
            btd_1.style.display ="flex";
            btd_1.style.justifyContent ="flex-end";
            btd_1.setAttribute("colspan","2");
            btr.appendChild(btd_1);
            var btddiv = document.createElement("div");
            // btddiv.className = "pagination right pull-right";
            btddiv.className = "pagination";
            btddiv.style.margin ="22px 0 10px 0";
            btd_1.appendChild(btddiv);
            var divul = document.createElement("ul");
            divul.style.paddingLeft ="0px";
            btddiv.appendChild(divul);
            ...
            lia.href = "javascript:void(0);";
            lia.style.color = "#aaa";
            lia.style.border = "1px solid transparent";
            lia.style.outline = "none";
            lia.style.textDecoration = "none";
            ulli.appendChild(lia);
            // lia.innerHTML = '共&nbsp;' + totalRecords + '&nbsp;条&nbsp;共&nbsp;' + totalPages + '&nbsp;页';
            lia.innerHTML = '共&nbsp;' + totalRecords + '&nbsp;条&nbsp;';
            var divul_2 = document.createElement("ul");
            divul_2.id="dataConts"
            divul_2.className = "dataTables_paginate paging_bootstrap pagination";
            // divul_2.className = "dataTables_paginate paging_bootstrap pagination pull-right";
            btd_1.appendChild(divul_2);
            ...
            if (pageNow > 1) {
                ...
                lia_2.id = "pagNum_" + (pageNow - 1);
                lia_2.href = "javascript:void(0);";
                lia_2.innerHTML = '<i id="pagNum_'+(pageNow - 1)+'" class="fa fa-arrow-left"></i>';
                ulli_2.appendChild(lia_2);
            } else {
                var ulli_2 = document.createElement("li");
                ulli_2.className = "prev disabled";
            ...
            if (pageNow >= totalPages) {
                ...
                lia_7.href = "javascript:void(0);";
                lia_7.id = "pagNum_" + (pageNow + 1);
                lia_7.innerHTML = '<i id="pagNum_'+(pageNow + 1)+'"  class="fa fa-arrow-right"></i>';
                ulli_7.appendChild(lia_7);
            };

            // 自定义跳转分页
            // var jumpDiv = document.createElement("div");
            // jumpDiv.className = 'jumpDiv';
            // btd_1.appendChild(jumpDiv);
            // var jumpSpan1 = document.createElement("span");
            // jumpSpan1.innerHTML = '当前页显示';
            // jumpDiv.appendChild(jumpSpan1);
            // var inputDiv = document.createElement("input");
            // inputDiv.className = 'jump';
            // inputDiv.value = params.pageSize || 10;
            // jumpDiv.appendChild(inputDiv);
            // var jumpSpan2 = document.createElement("span");
            // jumpSpan2.innerHTML = '条';
            // jumpDiv.appendChild(jumpSpan2);
            // var jumpSure = document.createElement("button");
            // jumpSure.className = 'jumpSure';
            // jumpSure.innerHTML = '确定';
            // jumpDiv.appendChild(jumpSure);
            var jumpDiv = document.createElement("div");
            jumpDiv.className = 'jumpDiv';
            btd_1.appendChild(jumpDiv);
            var jumpSpan1 = document.createElement("span");
            jumpSpan1.innerHTML = '跳至';
            jumpDiv.appendChild(jumpSpan1);
            var inputDiv = document.createElement("input");
            inputDiv.className = 'jump';
            inputDiv.value = pageNow;
            jumpDiv.appendChild(inputDiv);
            var jumpSpan2 = document.createElement("span");
            jumpSpan2.innerHTML = '页';
            jumpDiv.appendChild(jumpSpan2);
            var jumpSure = document.createElement("button");
            jumpSure.className = 'jumpSure';
            jumpSure.innerHTML = '确定';
            jumpSure.onclick = page_new.bind();
            jumpDiv.appendChild(jumpSure);

            $(jumpSure).on('click',function(){

                var reg=/(^[1-9]{1}[0-9]*$)/
                if(reg.test($(inputDiv).val())){
                    params.pageSize=$(inputDiv).val();
                    params.pageSize;
                }else{
                    layer.msg('只能输入大于0的数字');
                };
                loadData();
                        ...
                        if (!column[o].hide || column[o].hide == undefined) {
                            ...
                            if (l_tree.tree) {
                                ...
                                if (CommnUtil.in_array(lt, column[o].colkey)) {
                                    var divtree = document.createElement("div");
                                    divtree.className = "ly_tree";
                                    divtree.setAttribute("style", "float:left;padding-top:5px;margin-left:5px;text-align:"
        ...
        var setBgColor = function(tr) { // 设置背景色
            for (var i = 0; i < tr.childNodes.length; i++) {
                tr.childNodes[i].style.backgroundColor = "#dbeeff";
            }
        };
        ...
        var setDblclick = function(obj) {
            if (conf.dblclick != '') {
                obj.ondblclick = function(event){
                    $(this).parent().find("input[_l_key='checkbox']").prop('checked', false);
                    $(this).find("input[_l_key='checkbox']").prop('checked', true);
                    runFun(conf.dblclick);
                };
            }
        }
        var setColResizable = function() {
            if (conf.colResizable) {
...
function runFun(funcName) {
    if (typeof (eval(funcName)) == "function") {
        var s = eval(funcName + "();");
        return s;
    } else {
        // 函数不存在
        alert("指定函数(" + funcName + ")不存在!");
    }
